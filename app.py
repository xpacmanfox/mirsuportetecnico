...
# Inicializa o estado da escolha do sistema, caso não exista
...
# --- SISTEMA 1: LOCOMOTIVAS ---
elif st.session_state.sistema_ativo == "locomotivas":
    # Botão para voltar ao menu principal na barra lateral
    with st.sidebar:
        ...
        # (Aqui entra todo o código do seu app de locomotivas: abas, chat, etc.)
    
    st.markdown("# 🚂 Módulo de Locomotivas")
    st.write("Aqui roda o código voltado para locomotivas...")
    # Cole o restante do código da sua aplicação de locomotivas aqui
import os
from pathlib import Path
import streamlit as st
import chromadb
...
# --- SISTEMA 2: MÁQUINAS DE VIA ---
    ...
    with st.sidebar:
        if st.button("🏠 Voltar ao Menu Principal", use_container_width=True):
            ...
            st.rerun()
        st.divider()
        st.markdown("## 🛤️ MIR - Máquinas de Via")
        # (Aqui entra todo o código que acabamos de fazer para Máquinas de Via)
    
    st.markdown("# 🛤️ Módulo de Máquinas de Via")
    st.write("Aqui roda o código voltado para máquinas de via...")
    # Cole o restante do código da sua aplicação de máquinas de via aqui
...
# --- SIDEBAR MODERNA ---
with st.sidebar:
    st.markdown("## 🚆 MIR - Maquinas de Via")
    st.caption("Inteligência em Manutenção")
    st.divider()
